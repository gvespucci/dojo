...
public class CommandByEqualsMIDlet extends MIDlet implements ActionListener {
	...
	public CommandByEqualsMIDlet() {
		Display.init(this);
	}
	
	protected void startApp() throws MIDletStateChangeException {
		Form form = new Form("Command-By-Equals-MIDlet");
	...
	public void actionPerformed(ActionEvent event) {
		...
		if(commandId == EXIT_COMMAND_ID) {
			notifyDestroyed();
		} else	
		if(commandId == SAY_YES_COMMAND_ID) {
			sayYes();
		} else
		if(commandId == SAY_NO_COMMAND_ID) {
			sayNo();
		} else
		if(commandId == SAY_MAYBE_COMMAND_ID) {
			sayMaybe();
		}		
	}

	private void sayMaybe() {
		Dialog.show("Maybe Command", "Maybe... :|", OK, CANCEL);
	}

	private void sayNo() {
		Dialog.show("No Command", "No! :(", OK, CANCEL);
	}

	private void sayYes() {
		Dialog.show("Yes Command", "Yes! :)", OK, CANCEL);
	}
}
